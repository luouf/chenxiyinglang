...
public class SysMovieController {
    ...
    @Log(title="根据id去移除一个影视",businessType = BusinessType.DELETE)
    ...
    // 添加影视
    @Log(title="添加影视",businessType = BusinessType.INSERT)
    ...
    public Result addRole(@RequestBody SysMovie sysMovie)
    {
        ...
        if (res)
        {
            System.out.println("sysMovie.getId() = " + sysMovie.getId());
    ...
    // 实现修改
    @Log(title="修改影视",businessType = BusinessType.UPDATE)
    ...
    // 批量删除
    @Log(title="批量删除影视",businessType = BusinessType.DELETE)
